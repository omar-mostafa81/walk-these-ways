...
class Go2Parkour(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        # Power tensor
        self.power =  torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.power_friction =  torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.power_joule =  torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.power_work =  torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def resample_commands(self, env_ids):
        ...
        # Sample the command from the unit disk
        #angles = 2 * torch.tensor(np.pi) * torch.rand((len(env_ids,), device=self.device)
        #distances = torch.sqrt(torch.rand((len(env_ids),), device=self.device)) # If you want uniform sample in the unit disk
        #distances = torch.rand((len(env_ids),), device=self.device) # If you want uniform sample in the power dimension
