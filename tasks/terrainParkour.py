        ...
        for j in range(num_terrains):
            for i in range(num_levels):
                ...
                if self.terrain_keys[k] == "gap_parkour":
                    if self.easy_mode:
                        if i <= 2:
                            gap_length = 0.15 + i*0.1
                        else:
                            gap_length = 0.2 + 0.05*(i+1)
                    else:
                        if i <= 4:
                            gap_length = 0.15 + i*0.1 
                        else:
                            gap_length = 0.05*(i+1) + 0.3
                    ...
                    gap_parkour(
                        ...
                        gap_length=gap_length, #0.2 + 0.65*difficulty,
                    ...
                    ceiling = 0.34 - 0.08*difficulty # 0.1
                    boxes = crawl_parkour(
                        terrain,
                        lava_depth=lava_depth,
                        height=ceiling
                ...
                env_origin_y = (j + 0.5) * self.env_width
                env_origin_z = 0 # np.random.uniform(0.0, 0.4)
                self.env_origins[i, j] = [env_origin_x, env_origin_y, env_origin_z]
...
def crawl_parkour_old(terrain, platform_length=1.5, lava_width=0.5, lava_depth=-1.0, height=0.2, depth=1.0, width=3.0, height_step=0.1):
    # First put the barriers
    boxes = []
    boxes += box_trimesh(np.array([depth, width, 0.5]), np.array([platform_length+0.5*depth, 0.0,  height+0.25])),
    boxes += box_trimesh(np.array([depth, width, 0.5]), np.array([2*platform_length+1.5*depth, 0.0, height+0.25+height_step])),
    boxes += box_trimesh(np.array([depth, width, 0.5]), np.array([3*platform_length+2.5*depth, 0.0, height+0.25+2*height_step])),
    ...
    terrain.height_field_raw[:, 2*platform_length+1*depth:2*platform_length+2*depth] = 1*height_step
    terrain.height_field_raw[:, 3*platform_length+2*depth:3*platform_length+3*depth] = 2*height_step
