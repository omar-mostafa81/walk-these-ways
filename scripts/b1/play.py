...
def load_policy(path):
    body = torch.jit.load(os.path.join(path, 'checkpoints/body_latest.jit'))
    adaptation_module = torch.jit.load(os.path.join(path,'checkpoints/adaptation_module_latest.jit'))
...
def load_env(path, headless=False):
    with open(os.path.join(path, "parameters.pkl"), 'rb') as file:
    ...
    from gym_learn.ppo_cse.actor_critic import ActorCritic
